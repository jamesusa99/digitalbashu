...
const digitalIchCourses = [
  {
    title: "典籍里的首饰",
    desc: "以「神秘庄重的商周美学」「金声玉振的秦汉风貌」「花影重重的唐宋盛况」「珠光翠影的明清姿容」四个版块为框架，以《诗经》《楚辞》、唐诗、宋词、明清小说中所描写的饰物为知识载体，通过对饰物的研究衍生出对作品的趣味性解读，集趣味、美学、历史文化于一体。",
  },
  {
    title: "丹漆成器",
    desc: "深入探索中国传统漆器艺术的历史文化背景、制作工艺和艺术价值。通过介绍「髹漆」「漆金」「贴金箔」「镶嵌」等工艺技艺，理论与实践相结合，使学生全面了解漆器的起源、发展历程、制作材料、工艺技巧以及在现代社会的应用。",
  },
  {
    title: "指尖鞠舞",
    desc: "选取「五色之鞠」「传统之鞠」「字形之鞠」「年节之鞠」四个版块为框架，以始于原始社会、盛于唐宋的「鞠」为核心，通过技艺体验与创作，将中国传统手工技艺以复原再现的视角展示，集历史文化、古典美学与亲临体验于一体。",
  },
  {
    title: "布语时光",
    desc: "选取「福运绵绵」「福田悠悠」「衣被衿衿」「我心叩叩」四个拼布版块为框架，带领学生从实践角度体验中华传统文化技艺之美。传统拼布工艺有贴绢、百衲、堆绫、补花、拼花等，精湛技艺至今仍为30多个民族所传承。",
  },
  {
    title: "时光织语",
    desc: "选取「手织平纹」「布里生花」「线的翻转」「布缕不停」四个版块为框架，以传统非遗平面手织技艺、少数民族立体手织技艺、卡织技艺及当代复原的排织技艺为载体，使学生身临其境感受千年技艺，集历史文化、古典美学与亲临体验于一体。",
  },
  {
    title: "锦绣华章",
    desc: "选取「云纹之绣」「蝴蝶之绣」「民族之绣」「巴蜀之绣」四个版块为框架，以刺绣技艺演示、民族图腾化讲授、扇文化与巴蜀文化在刺绣中的体现等为载体，将中国非遗刺绣这颗「东方艺术明珠」以复原再现的视角展示出来。",
  },
  {
    title: "珠韵绣梦",
    desc: "从「团扇珠绣」「相框珠绣」「胸针珠绣」「发梳珠绣」等方面入手，让学生在学习珠绣文化的同时，通过手工实践掌握非遗珠绣制作的基本技巧。珠绣是有着悠久历史的中国传统手工艺，广泛应用于服装、鞋帽、箱包、挂饰等领域。",
  },
  {
    title: "中药造物",
    desc: "以中药为载体，构建「中药之源-产区地图」「中药之味-正气香牌」「中药之养-养生艾绒锤」「中药之变-北京非遗毛猴」「中药之美-文创首饰」五大核心板块，集知识性、艺术性与实践性于一体，引领学生体验中华医药传统文化的广博魅力。",
  },
  {
    title: "木意匠心",
    desc: "讲解木艺基础知识、工具使用、设计构思及项目实施等内容，通过古代门牌、鲁班智锁、创意花插等主题，将理论与实践相结合，让学生在学习「木艺制作技艺」的同时感受中华民族的智慧与匠人精神。",
  },
  {
    title: "传统文化",
    desc: "以「活字印刷」「华容道」「皮影戏」三大主题为核心，从传统技艺与艺术出发深入体验中华文化的独特魅力。在活字印刷中感受汉字之美，在华容道中体验逻辑与策略，在皮影戏的光影流转中品味戏曲故事与古典艺术。",
  },
];
  ...
  return (
    <>
      ...
      <section className="bg-paper py-14 sm:py-16">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            <h2 className="font-serif text-2xl font-bold text-ink sm:text-3xl">数字非遗课程</h2>
            <p className="mt-1 text-muted">研究院非遗数字化精品课程体系 · 传统技艺活态传承</p>
          </div>
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {digitalIchCourses.map((course) => (
              <div
                key={course.title}
                className="flex flex-col rounded-2xl border border-border bg-snow p-6 shadow-sm transition hover:border-bashu-bronze/40 hover:shadow-md"
              >
                <h3 className="font-serif text-base font-semibold text-ink">{course.title}</h3>
                <p className="mt-3 flex-1 text-sm text-muted leading-relaxed">{course.desc}</p>
              </div>
            ))}
          </div>
        </div>
